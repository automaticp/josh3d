...
struct BufferAttachments
{
    ...
    // Wraps `glVertexArrayElementBuffer` with `buffer = element_buffer`.
    template<of_kind<GLKind::Buffer> BufferT>
    void attach_element_buffer(const BufferT& element_buffer) const requires mt::is_mutable
    {
        gl::glVertexArrayElementBuffer(self_id(), decay_to_raw(element_buffer).id());
    }

    // Wraps `glVertexArrayElementBuffer` with `buffer = 0`
    void detach_element_buffer() const requires mt::is_mutable
    {
        gl::glVertexArrayElementBuffer(self_id(), 0);
    }

    // Wraps `glVertexArrayVertexBuffer`
    template<of_kind<GLKind::Buffer> BufferT>
    void attach_vertex_buffer(
        VertexBufferSlot buffer_slot,
        const BufferT&   buffer,
        OffsetBytes      offset_bytes,
        StrideBytes      stride_bytes) const
            requires mt::is_mutable
    {
        ...
        */
        ...
        gl::glVertexArrayVertexBuffer(
            self_id(), buffer_slot, decay_to_raw(buffer).id(),
            offset_bytes, stride_bytes);
    }

    void detach_vertex_buffer(VertexBufferSlot buffer_slot) const requires mt::is_mutable
    {
        gl::glVertexArrayVertexBuffer(self_id(), buffer_slot, 0, 0, 0);
    }
    ...
    void attach_vertex_buffers(
        ...
        std::span<const GLsizei>  buffer_strides_bytes) const
            requires mt::is_mutable
    {
...
struct AttributeSpecification
{
    ...
    void specify_float_attribute(
        ...
        OffsetBytes         offset_bytes = OffsetBytes{ 0 }) const
            requires mt::is_mutable
    {
    ...
    //
    // TODO: This could be a free function in ObjectHelpers instead.
    template<specializes_attribute_traits VertexT>
    auto specify_custom_attributes(AttributeIndex first_attrib_index) const -> GLsizeiptr
        requires mt::is_mutable
    {
...
template<typename CRTP>
struct BindingToAttributeAssociation
{
    JOSH3D_MIXIN_HEADER

    void associate_attribute_with_buffer_slot(
        AttributeIndex   attrib_index,
        VertexBufferSlot buffer_slot) const
            requires mt::is_mutable
    {
...
template<typename CRTP>
struct EnableDisableAttributeAccess
{
    JOSH3D_MIXIN_HEADER
    ...
    void enable_attribute(AttributeIndex attrib_index) const requires mt::is_mutable
    {
        gl::glEnableVertexArrayAttrib(self_id(), attrib_index);
    }

    void disable_attribute(AttributeIndex attrib_index) const requires mt::is_mutable
    {
...
struct Queries
{
    ...
    auto get_attached_element_buffer_id() const -> GLuint
    {
        return get_integer(gl::GL_ELEMENT_ARRAY_BUFFER_BINDING);
    }

    auto get_attached_vertex_buffer_offset_bytes(VertexBufferSlot buffer_slot) const -> OffsetBytes
    {
        return get_integer64_indexed(gl::GL_VERTEX_BINDING_OFFSET, buffer_slot);
    }

    auto get_attached_vertex_buffer_stride_bytes(VertexBufferSlot buffer_slot) const -> StrideBytes
    {
        return get_integer_indexed(gl::GL_VERTEX_BINDING_STRIDE, buffer_slot);
    }

    auto get_attached_vertex_buffer_id(VertexBufferSlot buffer_slot) const -> GLuint
    {
        return get_integer_indexed(gl::GL_VERTEX_BINDING_BUFFER, buffer_slot);
    }

    auto get_buffer_slot_divisor(VertexBufferSlot buffer_slot) const -> GLuint
    {
        return get_integer_indexed(gl::GL_VERTEX_BINDING_DIVISOR, buffer_slot);
    }


    auto is_attribute_enabled(AttributeIndex attrib_index) const -> bool
    {
        return get_boolean_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_ENABLED, attrib_index);
    }

    auto get_attribute_components(AttributeIndex attrib_index) const -> AttributeComponentsAll
    {
        return AttributeComponentsAll{ get_integer_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_SIZE, attrib_index) };
    }

    auto get_attribute_type(AttributeIndex attrib_index) const -> AttributeType
    {
        return enum_cast<AttributeType>(get_enum_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_TYPE, attrib_index));
    }

    auto is_attribute_normalized(AttributeIndex attrib_index) const -> bool
    {
        return get_boolean_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, attrib_index);
    }

    auto is_attribute_integer(AttributeIndex attrib_index) const -> bool
    {
        return get_boolean_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_INTEGER, attrib_index);
    }

    auto is_attribute_double(AttributeIndex attrib_index) const -> bool
    {
        return get_boolean_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_LONG, attrib_index);
    }


    auto get_associated_slot_stride_bytes(AttributeIndex attrib_index) const -> StrideBytes {
        return StrideBytes{ get_integer_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_STRIDE, attrib_index) };
    }

    auto get_associated_slot_divisor(AttributeIndex attrib_index) const -> GLuint
    {
        return get_integer_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_DIVISOR, attrib_index);
    }

    auto get_associated_slot_offset_bytes(AttributeIndex attrib_index) const -> OffsetBytes
    {
        return OffsetBytes{ get_integer_indexed(gl::GL_VERTEX_ATTRIB_RELATIVE_OFFSET, attrib_index) };
    }

    auto get_associated_slot(AttributeIndex attrib_index) const -> VertexBufferSlot
    {
        return VertexBufferSlot{ get_integer_indexed(gl::GL_VERTEX_ATTRIB_BINDING, attrib_index) };
    }

    // TODO: I am SOOOOO unsure about this. Does this return the NAME of the buffer?
    auto get_associated_slot_buffer_id(AttributeIndex attrib_index) const -> GLuint
    {
        return get_integer_indexed(gl::GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, attrib_index);
    }

private:
    auto get_integer(GLenum pname) const
    {
        GLint result; gl::glGetVertexArrayiv(self_id(), pname, &result); return result;
    }

    auto get_boolean_indexed(GLenum pname, GLuint index) const
    {
        GLboolean result; gl::glGetVertexArrayIndexediv(self_id(), index, pname, &result); return result;
    }

    auto get_integer_indexed(GLenum pname, GLuint index) const
    {
        GLint result; gl::glGetVertexArrayIndexediv(self_id(), index, pname, &result); return result;
    }

    auto get_enum_indexed(GLenum pname, GLuint index) const
    {
        GLenum result; gl::glGetVertexArrayIndexediv(self_id(), index, pname, &result); return result;
    }

    auto get_integer64_indexed(GLenum pname, GLuint index) const
    {
        GLint64 result; gl::glGetVertexArrayIndexed64iv(self_id(), index, pname, &result); return result;
    }
};
...
struct Bind
{
    ...
    auto bind() const -> BindToken<Binding::VertexArray>
    {
...
template<typename CRTP>
struct VertexArray
    : BufferAttachments<CRTP>
    , AttributeSpecification<CRTP>
    , BindingToAttributeAssociation<CRTP>
    , EnableDisableAttributeAccess<CRTP>
    , Queries<CRTP>
    , Bind<CRTP>
{
    JOSH3D_MIXIN_HEADER
...
class RawVertexArray
    ...
    , public detail::vertex_array_api::VertexArray<RawVertexArray<MutT>>
{
public:
    static constexpr auto kind_type = GLKind::VertexArray;
    JOSH3D_MAGIC_CONSTRUCTORS_2(RawVertexArray, mutability_traits<RawVertexArray>, detail::RawGLHandle<>)
};
