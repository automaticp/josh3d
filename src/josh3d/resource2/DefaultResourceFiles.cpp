...
template<typename T>
auto ptr_at_offset(const MappedRegion& mregion, usize offset_bytes) noexcept
    -> T*
{
...
void throw_if_too_small_for_header(usize file_size) noexcept(false)
{
    if (file_size < sizeof(HeaderT))
        throw InvalidResourceFile("Resource file is too small to contain header information.");
}

void throw_on_unexpected_size(usize expected, usize real) noexcept(false)
{
    if (real != expected)
        throw_fmt<InvalidResourceFile>("Resource file unexpected size. Expected {}, got {}.", expected, real);
}
...
[[nodiscard]]
constexpr auto next_aligned(uintptr value, usize alignment)
    -> uintptr
{
...
auto SkeletonFile::joints() const noexcept
    -> Span<Joint>
{
    const usize offset = sizeof(Header);
...
auto SkeletonFile::required_size(const Args& args) noexcept
    -> usize
{
    const usize num_joints  = args.num_joints;
    const usize size_header = sizeof(Header);
    const usize size_joints = sizeof(Joint) * num_joints;
    const usize size_names  = sizeof(ResourceName) * num_joints;
    const usize total_size  = size_header + size_joints + size_names;
    return total_size;
}
...
{
    SkeletonFile file{ MOVE(mapped_region) };
    const usize file_size     = file.size_bytes();
    const usize expected_size = required_size({ .num_joints = file.header().num_joints });
...
auto AnimationFile::joint_span(uindex joint_id) const noexcept
    -> JointSpan&
{
...
auto AnimationFile::keyframes_header(uindex joint_id) const noexcept
    -> KeyframesHeader&
{
    assert(joint_id < header().num_joints);
...
auto AnimationFile::pos_keys(uindex joint_id) const noexcept
    -> Span<KeyVec3>
{
    assert(joint_id < header().num_joints);
...
auto AnimationFile::rot_keys(uindex joint_id) const noexcept
    -> Span<KeyQuat>
{
    assert(joint_id < header().num_joints);
    const auto& kfs = keyframes_header(joint_id);
...
auto AnimationFile::sca_keys(usize joint_id) const noexcept
    -> Span<KeyVec3>
{
    assert(joint_id < header().num_joints);
    const auto& kfs = keyframes_header(joint_id);
...
auto AnimationFile::required_size(const Args& args) noexcept
    -> usize
{
    const u16   num_joints       = args.key_specs.size();
    const usize header_size      = sizeof(Header);
    const usize joint_spans_size = num_joints * sizeof(JointSpan);

    usize all_keyframes_size = 0;
    ...
    -> AnimationFile
{
    ...
    usize current_offset = sizeof(Header) + num_joints * sizeof(JointSpan);

    for (uindex joint_id{ 0 }; joint_id < num_joints; ++joint_id)
    {
        ...
        span = {
            .offset_bytes = u32(current_offset),
            .size_bytes   = u32(keyframes_size(spec)),
        };
    ...
    -> AnimationFile
{
    ...
    const usize num_joints      = file.header().num_joints;
    const usize with_spans_size = sizeof(Header) + num_joints * sizeof(JointSpan);
    ...
    const usize expected_size = last_span.offset_bytes + last_span.size_bytes;
    throw_on_unexpected_size(expected_size, file_size);
...
auto StaticMeshFile::lod_span(usize lod_id) const noexcept
    -> LODSpan&
{
...
auto StaticMeshFile::lod_verts_bytes(usize lod_id) const noexcept
    -> Span<ubyte>
{
    assert(lod_id < header().num_lods);
    const LODSpan& span = lod_span(lod_id);
    const usize offset = span.verts_offset_bytes;
    return { ptr_at_offset<ubyte>(mregion_, offset), span.verts_size_bytes };
}

auto StaticMeshFile::lod_elems_bytes(usize lod_id) const noexcept
    -> Span<ubyte>
{
    assert(lod_id < header().num_lods);
    const LODSpan& span = lod_span(lod_id);
    const usize  offset = span.elems_offset_bytes;
...
auto StaticMeshFile::required_size(const Args& args) noexcept
    -> usize
{
    usize total_size = sizeof(Header);
    ...
    -> StaticMeshFile
{
    ...
    const usize num_lods = args.lod_specs.size();
    ...
    usize current_offset = sizeof(Header);
    for (usize lod_id{ num_lods }; lod_id --> 0;)
    {
    ...
    -> StaticMeshFile
{
    ...
    usize expected_size = sizeof(Header);
    for (usize lod_id{ 0 }; lod_id < header.num_lods; ++lod_id)
    {
        const usize verts_bytes = header.lods[lod_id].verts_size_bytes;
        const usize elems_bytes = header.lods[lod_id].elems_size_bytes;
...
auto SkinnedMeshFile::lod_span(usize lod_id) const noexcept
    -> LODSpan&
{
    assert(lod_id < header().num_lods);
    return header().lods[lod_id];
...
auto SkinnedMeshFile::lod_verts_bytes(usize lod_id) const noexcept
    -> Span<ubyte>
{
    assert(lod_id < header().num_lods);
    const LODSpan& span = lod_span(lod_id);
...
auto SkinnedMeshFile::lod_elems_bytes(usize lod_id) const noexcept
    -> Span<ubyte>
{
    assert(lod_id < header().num_lods);
    const LODSpan& span = lod_span(lod_id);
...
auto TextureFile::mip_span(usize mip_id) const noexcept
    -> MIPSpan&
{
...
auto TextureFile::mip_bytes(usize mip_id) const noexcept
    -> Span<ubyte>
{
    assert(mip_id < header().num_mips);
    const MIPSpan& span = mip_span(mip_id);
    const usize offset = span.offset_bytes;
    ...
    -> TextureFile
{
    ...
    const usize num_mips = args.mip_specs.size();
    ...
    usize current_offset{ sizeof(Header) };
    for (usize mip_id{ num_mips }; mip_id --> 0;)
    {
    ...
    -> TextureFile
{
    ...
    for (const usize mip_id : irange(header.num_mips))
        if (not enum_valid(file.mip_span(mip_id).encoding))
            throw InvalidResourceFile("Texture file has invalid encoding.");

    // Check size.
    usize expected_size = sizeof(Header);
    for (usize mip_id{ 0 }; mip_id < header.num_mips; ++mip_id)
    {
        const usize size_bytes = header.mips[mip_id].size_bytes;
