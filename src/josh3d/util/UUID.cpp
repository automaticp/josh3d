...
auto generate_uuid() noexcept -> UUID
{
...
auto deserialize_uuid(StrView string_repr) -> UUID
{
...
void serialize_uuid_to(Span<char, 36> out_buf, const UUID& uuid)
{
    const bool success [[maybe_unused]] =
        boost::uuids::to_chars(uuid, out_buf.data(), out_buf.data() + out_buf.size());
    assert(success);
}

void serialize_uuid_to(Span<char, 37> out_buf, const UUID& uuid)
{
    const bool success [[maybe_unused]] =
        boost::uuids::to_chars(uuid, out_buf.data(), out_buf.data() + out_buf.size());
    assert(success);
...
auto serialize_uuid(const UUID& uuid) -> String
{
