...
constexpr vec3 X = { 1.f, 0.f, 0.f };
constexpr vec3 Y = { 0.f, 1.f, 0.f };
constexpr vec3 Z = { 0.f, 0.f, 1.f };
...
inline auto Quad::transformed(const mat4& world_mat) const noexcept
    -> Quad
{
    return {
        world_mat * vec4{ points[0], 1.f },
        world_mat * vec4{ points[1], 1.f },
        world_mat * vec4{ points[2], 1.f },
        world_mat * vec4{ points[3], 1.f },
    };
}
...
inline auto Plane::transformed(const mat4& world_mat) const noexcept
    -> Plane
{
    const mat3 L2W_mat3 = inverse(world_mat);
    const vec3 position = world_mat[3];
    ...
    const vec3 new_normal = normalize(normal * L2W_mat3);
