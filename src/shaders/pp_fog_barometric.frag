...
void main() {
    ...
    vec4 world_up_vs = normalize(camera.view * vec4(0.0, 1.0, 0.0, 0.0));

    float transmittance = transmittance(
        pos_vs.xyz,
        screen_z,
        scale_height,
        density_at_eye_height,
        world_up_vs.xyz
    );
    ...
    vec3  world_up_vs)
{
    ...
    */
    ...
    float dY = -dot(frag_pos_vs, world_up_vs);
    ...
    float dR = max(length(frag_pos_vs), abs(dY));
